...
def relatorio(request):
    ...
    if request.GET['tipo_relatorio'] == '0' :#faturamento
        ...
        for conta_entrada in contas_entrada:
            transacoes += Transacao.objects.filter(pago = 0, conta = conta_entrada).order_by('data_vencimento')#mudar pago para um
        ...
        for conta_saida in contas_saida:
            transacoes += Transacao.objects.filter(pago = 0, conta = conta_saida).order_by('data_vencimento')#mudar pago para um
        ...
        contas_saida = Conta.objects.filter(tipo=1)
        for conta_entrada in contas_entrada:
            transacoes += Transacao.objects.filter(pago = 0, conta = conta_entrada).order_by('data_vencimento')#mudar pago para 1
        for conta_saida in contas_saida:
            transacoes_saida += Transacao.objects.filter(pago = 0, conta = conta_saida).order_by('data_vencimento')#mudar pago para 1
    ...
    return render_to_response("relatorio.html", {'titulo': titulo_grafico,'anos':anos,'dados_anos':dados,'meses_duracao':meses_duracao,'tipo_grafico' : tipo_grafico,'transacoes':transacoes,'transacoes_saida':transacoes_saida},context_instance=RequestContext(request));
